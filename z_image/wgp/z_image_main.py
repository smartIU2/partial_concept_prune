...
def get_config(model_type):
    transformer_config = os.path.join(os.path.dirname(os.path.abspath(__file__)), "configs", f"{model_type}.json")
...
class model_factory:
    def __init__(
        ...
        **kwargs,
        ...
        # Tokenizer
        if tokenizer_folder is None or tokenizer_folder == "":
            tokenizer_folder = str(Path(text_encoder_filename).parent)
        
        tokenizer = AutoTokenizer.from_pretrained(tokenizer_folder, trust_remote_code=True)

        # VAE
        if vae_config is None or vae_config == "":
            vae_config = os.path.splitext(vae_filename)[0] + "_config.json"
        ...
        # Scheduler
        with open(scheduler_filename, "r", encoding="utf-8") as f:
    ...
    def generate(
        ...
        **kwargs,
        ...
        generator = torch.Generator(device="cuda" if force_cpu==False and torch.cuda.is_available() else "cpu")
